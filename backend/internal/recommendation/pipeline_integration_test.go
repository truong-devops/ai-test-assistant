...
func TestRecommendationPipelineWithProjectRAGAndMockProvider(t *testing.T) {
	...
	defer pool.Close()
	projectID, analysisID, symbolID := createRecommendationAnalysis(t, ctx, pool, "pipeline")
	defer func() { _, _ = pool.Exec(context.Background(), `DELETE FROM projects WHERE id=$1`, projectID) }()

	embedder := knowledge.NewHashEmbeddingClient("hash-pipeline", knowledge.EmbeddingDimensions)
	chunks := []struct{ key, path, pkg, symbol, kind, content string }{
		{"impl", "internal/user/service.go", "user", "CreateUser", "method", "func (s *Service) CreateUser(email string) error { return s.repository.Create(email) }"},
		{"interface", "internal/user/service.go", "user", "Repository", "interface", "type Repository interface { Create(string) error }"},
		{"test", "internal/user/service_test.go", "user", "TestCreateUser", "test", "func TestCreateUser(t *testing.T) { /* success case */ }"},
		{"mock", "internal/user/mock_repository_test.go", "user", "mockRepository", "mock", "type mockRepository struct { createCalls int }"},
	}
	for _, chunk := range chunks {
		...
		if _, err := pool.Exec(ctx, `INSERT INTO knowledge_chunks
			...
			VALUES ($1,$2,$3,$4,$5,$6,$7,$8,1,1,'hash-pipeline',$9::vector,'{}')`,
			projectID, chunk.key, chunk.path, chunk.pkg, chunk.symbol, chunk.kind,
			chunk.content, knowledge.ContentHash(chunk.content), integrationVector(vectors[0])); err != nil {
			t.Fatal(err)
		}
	}
	...
	provider := &providerStub{result: llm.Response{
		ID: "resp-pipeline", Model: "fixture-model", Output: validProviderOutput,
	}}
	processor := NewProcessor(job.NewRepository(pool),
	...
	if err != nil || len(items) != 1 || items[0].ChangedSymbolID != symbolID ||
		items[0].ProviderResponseID != "resp-pipeline" {
	...
	if err != nil || analysisJob.Status != job.StatusGeneratingTests {
		t.Fatalf("analysis=%#v error=%v", analysisJob, err)
	}
}

func integrationVector(values []float32) string {
