...
func writeDocumentExecutionError(w http.ResponseWriter, err error, fallback string) {
	switch {
	case errors.Is(err, report.ErrNotFound), errors.Is(err, scope.ErrNotFound), errors.Is(err, automation.ErrNotFound):
		writeError(w, http.StatusNotFound, err.Error())
	case errors.Is(err, report.ErrInvalidInput), errors.Is(err, scope.ErrInvalidInput), errors.Is(err, automation.ErrInvalidInput), errors.Is(err, automation.ErrInvalidOutput):
		writeError(w, http.StatusBadRequest, err.Error())
	case errors.Is(err, scope.ErrUnsafeChange), errors.Is(err, automation.ErrBlocked), errors.Is(err, automation.ErrAlreadyReviewed):
